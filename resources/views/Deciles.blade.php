...
@section('Content')
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            ...
            <h2 class="font-outfit text-3xl sm:text-4xl font-bold text-gray-900">Calculadora de Deciles</h2>
            <p class="text-gray-500 mt-2 text-sm">Divide tu conjunto de datos en diez partes iguales.</p>
        </div>

        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="p-6 sm:p-8">
                <form action="{{ route('Decile') }}" method="post" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        <label for="number" class="block text-sm font-semibold text-gray-700 mb-2">Numero de decil</label>
                        <select
                            name="number"
                            id="number"
                        ...
                        >
                            @for ($i = 1; $i < 10; $i++)
                            <option value="{{ $i }}">Decil {{ $i }} (D{{ $i }})</option>
                            @endfor
                        </select>
                    </div>
        ...
        <div class="mt-4 px-1">
            <div class="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-100">
                ...
                <p class="text-sm text-amber-700">Para obtener un mejor resultado es recomendable ingresar al menos 10 datos.</p>
            </div>
        </div>
    </div>
@endsection
