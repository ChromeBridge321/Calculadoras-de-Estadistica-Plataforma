...
@section('Content')
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            ...
            <h2 class="font-outfit text-3xl sm:text-4xl font-bold text-gray-900">Calculadora de Cuartiles</h2>
            <p class="text-gray-500 mt-2 text-sm">Divide tu conjunto de datos en cuatro partes iguales.</p>
        </div>

        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="p-6 sm:p-8">
                <form action="{{ route('Quartile') }}" method="post" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        <label for="number" class="block text-sm font-semibold text-gray-700 mb-2">Numero de cuartil</label>
                        <select
                            name="number"
                            id="number"
                        ...
                        >
                            <option value="1">1er Cuartil (Q1)</option>
                            <option value="2">2do Cuartil (Q2)</option>
                            <option value="3">3er Cuartil (Q3)</option>
                        </select>
                    </div>
            ...
            <div class="border-t border-gray-100 bg-red-50 p-6 sm:p-8 space-y-4">
                <div>
                    ...
                    <input
                        ...
                        value="{{ $data }}"
                        readonly
                        class="w-full px-4 py-3 rounded-xl border-0 bg-white/80 text-gray-900 font-mono text-sm shadow-sm focus:outline-none"
                    >
                </div>
                <div>
        ...
        <div class="mt-4 px-1">
            <div class="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-100">
                ...
                <p class="text-sm text-amber-700">Para obtener un mejor resultado es recomendable ingresar al menos 4 datos.</p>
            </div>
        </div>
    </div>
@endsection
